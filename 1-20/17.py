...
def numberToWords(number):
        if number < 100:
                return _NUMBER_NAMES[number]

        elif number % 100 == 0:
                return _NUMBER_NAMES[ int(str(number)[0]) ] + " hundred"

        else:
                tens = number%100
                        
        return _NUMBER_NAMES[ int(str(number)[0]) ] + " hundred and " + _NUMBER_NAMES[tens]

def numberOfCharactersWeCareAbout(phrase):
        return len(re.compile("\w").findall(phrase))


def valueOfNumber(number):

        return numberOfCharactersWeCareAbout(numberToWords(number))

print valueOfNumber(342)


total = 0

for i in range(1,1000):
        total += valueOfNumber(i)
